...
class Database {
  constructor() {
    this.dbPath = process.env.DATABASE_PATH || path.join(__dirname, '..', 'data', 'hamza_agent.db');
    this.db = null;
  ...
  async insertDefaultSettings() {
    const defaultSettings = [
      ...
      ['publish_time_optimization', 'false', 'Let the weekly review move approved publish times to preset peak hours'],
      ...
      ['max_daily_posts', '1', 'Maximum automatic uploads per day (0 = no limit)'],
      ['posting_frequency', 'daily', 'Generation cadence without an active strategy: daily, every-2-days, 3-per-week, weekly'],
    ...
    // Retired setting that never controlled anything.
    await this.executeQuery("DELETE FROM settings WHERE key = 'thumbnail_ab_testing'");
  ...
  async updateGenerationJob(id, changes = {}) {
    ...
    // details and cancel_requested are merged in SQL, not from the snapshot read above: a stage
    // update racing with an operator's cancel must not erase the cancel flag or its reason.
    await this.executeQuery(
      `UPDATE generation_jobs SET status = ?, stage = ?, progress = ?, production_id = ?,
        title = ?, error = ?, details = json_patch(COALESCE(details, '{}'), ?),
        cancel_requested = COALESCE(?, cancel_requested), updated_at = datetime('now'),
      ...
      [
        ...
        changes.cancelRequested === undefined ? null : Number(changes.cancelRequested),
  ...
  async markInterruptedJobs() {
    ...
    // A reply mid-post when the process died may or may not exist on YouTube.
    await this.executeQuery(
      `UPDATE reply_drafts SET status = 'uncertain',
       failure_reason = 'The application restarted while posting; check YouTube before retrying',
       updated_at = CURRENT_TIMESTAMP WHERE status = 'posting'`
    );
  }
  ...
  async updateOperatorRun(id, changes = {}) {
    ...
    await this.executeQuery(
      `UPDATE operator_runs SET status = ?, stage = ?, progress = ?, research = ?, plan = ?,
       generated_jobs = ?, summary = ?, error = ?, cancel_requested = COALESCE(?, cancel_requested),
      ...
      [
        ...
        changes.error === undefined ? current.error : changes.error,
        // Only an explicit change touches the flag, so progress updates cannot undo a cancel.
        changes.cancelRequested === undefined ? null : Number(changes.cancelRequested),
        changes.completedAt === undefined ? current.completed_at : changes.completedAt,
        id
      ]
  ...
  // Atomically move a draft into 'posting'. Only one caller can win, so a double-clicked
  // approval cannot post the same reply twice.
  async claimReplyDraftForPosting(id) {
    const result = await this.executeQuery(
      `UPDATE reply_drafts SET status = 'posting', failure_reason = NULL, updated_at = CURRENT_TIMESTAMP
  ...
  async countPublishedSince(isoTime) {
    const row = await this.getRow(
      "SELECT COUNT(*) AS count FROM publish_schedule WHERE status = 'published' AND published_at >= ?",
      [isoTime]
    );
