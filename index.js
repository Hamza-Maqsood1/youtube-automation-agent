...
class YouTubeAutomationAgent {
  constructor() {
    ...
    this.localApiKey = null;
    this.apiKeyPath = process.env.API_KEY_FILE || path.join(__dirname, 'data', '.api-key');
  }
  ...
  async logCapabilitySummary() {
    ...
    const capabilities = [
      ...
      { name: 'Voice narration (TTS)', ok: hasTTS, hint: 'configure OpenAI, Gemini, or ElevenLabs — otherwise videos are silent' },
  ...
  // Express 4 ignores rejected promises from async handlers; an unguarded database error would
  // hang the request and, under Node 15+, crash the process. Forward them to the error handler.
  wrapAsyncRoutes(app) {
    const wrap = handler => {
    ...
    for (const method of ['get', 'post', 'put', 'patch', 'delete']) {
      ...
      // app.get('setting') with a single argument is Express's settings getter; leave it alone.
      app[method] = (routePath, ...handlers) => handlers.length ? register(routePath, ...handlers.map(wrap)) : register(routePath);
    }
  }
  ...
  handleRouteError() {
    return (error, _req, res, next) => {
      ...
      if (status >= 500) this.logger.error('Unhandled API error:', error);
      return res.status(status).json({
        success: false,
        error: status >= 500 && !error.status ? 'Internal server error' : error.message,
        ...(error.code ? { code: error.code } : {})
      });
    };
  }

  // API_KEY from the environment, or a random per-install key persisted in data/.api-key.
  // Mutating routes are never left open: an empty API_KEY used to disable protection entirely.
  getApiKey() {
    if (process.env.API_KEY) return process.env.API_KEY;
    if (this.localApiKey) return this.localApiKey;
    try {
      this.localApiKey = fsSync.readFileSync(this.apiKeyPath, 'utf8').trim();
    } catch (_error) {
      this.localApiKey = null;
    }
    if (!this.localApiKey) {
      const generated = crypto.randomBytes(32).toString('hex');
      fsSync.mkdirSync(path.dirname(this.apiKeyPath), { recursive: true });
      try {
        fsSync.writeFileSync(this.apiKeyPath, `${generated}\n`, { mode: 0o600, flag: 'wx' });
        this.localApiKey = generated;
      } catch (error) {
  ...
  requireAPIKey() {
    return (req, res, next) => {
      const expected = crypto.createHash('sha256').update(String(this.getApiKey())).digest();
      const supplied = crypto.createHash('sha256').update(String(req.get('x-api-key') || '')).digest();
      if (!req.get('x-api-key') || !crypto.timingSafeEqual(expected, supplied)) {
  ...
  // Reject unexpected Host headers so a DNS-rebinding page cannot reach the local API.
  requireAllowedHost() {
    const allowed = new Set(['localhost', '127.0.0.1', '::1']);
    for (const host of String(process.env.ALLOWED_HOSTS || '').split(',')) {
    ...
    return (req, res, next) => {
      ...
      if (!allowed.has(hostname)) {
        return res.status(421).json({ success: false, error: 'Host not allowed. Add it to ALLOWED_HOSTS to serve the dashboard under this name.' });
      }
      return next();
    };
  }
  ...
  setupAPI() {
    this.app.use(this.requireAllowedHost());
    // Every state-changing request needs the key. Browsers cannot attach a custom header to a
    // cross-site form post, so this also blocks CSRF against the local dashboard.
    const protectMutations = this.requireAPIKey();
    this.app.use((req, res, next) => (
      ['GET', 'HEAD', 'OPTIONS'].includes(req.method) ? next() : protectMutations(req, res, next)
    ));
    this.app.use(express.json({ limit: '1mb' }));
    this.app.use(express.static(path.join(__dirname, 'dashboard')));

    // Hands the key to a dashboard opened on this machine. Cross-origin pages cannot read the
    // response (no CORS) and the Host check blocks DNS rebinding; LAN clients must enter it manually.
    this.app.get('/api/session', (req, res) => {
      res.set('Cache-Control', 'no-store');
      if (!this.isLoopbackRequest(req)) {
        return res.status(403).json({ success: false, error: 'Enter the API key manually when connecting from another device' });
      }
      return res.json({ success: true, apiKey: this.getApiKey() });
    });
    ...
    this.app.use(this.handleRouteError());
  }
  ...
  setupOperatorAPI() {
    ...
    this.app.put('/api/settings', protect, async (req, res) => {
      const settingRules = {
        approval_required: 'boolean', notification_enabled: 'boolean', daily_content_enabled: 'boolean',
        auto_publish_enabled: 'boolean', analytics_enabled: 'boolean', optimization_enabled: 'boolean',
        publish_time_optimization: 'boolean', content_backup_enabled: 'boolean',
        max_daily_posts: [0, 50], content_buffer_days: [0, 30],
        posting_frequency: ['daily', 'every-2-days', '3-per-week', 'weekly'], channel_timezone: 'timezone'
      };
      // Validate everything before writing anything, so a bad value cannot leave a partial update.
      const updates = [];
      for (const [key, rule] of Object.entries(settingRules)) {
        if (req.body?.[key] === undefined) continue;
        const value = String(req.body[key]).trim();
        const valid = rule === 'boolean' ? ['true', 'false'].includes(value)
          : rule === 'timezone' ? (() => { try { new Intl.DateTimeFormat('en-US', { timeZone: value }); return true; } catch (_error) { return false; } })()
          : Array.isArray(rule) && typeof rule[0] === 'number' ? /^\d+$/.test(value) && Number(value) >= rule[0] && Number(value) <= rule[1]
          : rule.includes(value);
        if (!valid) return res.status(400).json({ error: `Invalid value for ${key}` });
        updates.push([key, value]);
      }
      for (const [key, value] of updates) await this.db.setSetting(key, value);
      if (updates.some(([key]) => key === 'channel_timezone')) await this.scheduler?.rescheduleTasks?.();
  ...
  async startGenerationJob(input = {}) {
    ...
    // Claim the slot synchronously, before any await, so concurrent requests cannot all pass
    // the capacity check and exceed MAX_CONCURRENT_JOBS.
    const reservation = Symbol('reserved-generation-slot');
    this.activeJobs.set(reservation, null);
    let job;
    try {
      job = await this.db.createGenerationJob({
        ...validation.value,
        source: input.source || 'manual'
      });
    } finally {
      this.activeJobs.delete(reservation);
    }
  ...
  async resumeGenerationJob(jobId, options = {}) {
    ...
    // Reserve this job's slot before awaiting, so a double-clicked Resume cannot start it twice.
    this.activeJobs.set(job.id, null);
    try {
      return await this.launchResumedJob(job, options);
    } catch (error) {
      if (this.activeJobs.get(job.id) === null) this.activeJobs.delete(job.id);
      throw error;
    }
  }

  async launchResumedJob(job, options) {
  ...
  async generateContent(topic = null, style = null, length = 'medium', options = {}) {
    ...
    // `strategyContext` can arrive as a literal `null` (the API's "not provided" sentinel,
    // since JSON has no `undefined`) rather than simply being absent, which a destructuring
    // default only covers for `undefined`. Normalize explicitly so daily-scheduled runs with
    // no active channel strategy don't crash on `strategyContext.angle`.
  ...
  // A stray rejection in background work (jobs, cron tasks, notifications) is logged instead of
  // killing the server; a truly uncaught exception leaves state unknown, so shut down cleanly.
  installProcessHandlers() {
    process.on('unhandledRejection', reason => {
      this.logger.error('Unhandled promise rejection (process kept running):', reason instanceof Error ? reason : new Error(String(reason)));
    });
    process.on('uncaughtException', error => {
      this.logger.error('Uncaught exception; shutting down:', error);
      this.shutdown('uncaughtException', 1);
    });
    for (const signal of ['SIGINT', 'SIGTERM']) process.once(signal, () => this.shutdown(signal, 0));
  }

  async shutdown(reason, exitCode = 0) {
    if (this.shuttingDown) return;
    this.shuttingDown = true;
    this.logger.warn(`Shutting down (${reason})...`);
    const force = setTimeout(() => process.exit(exitCode), 10000);
    force.unref();
    try {
      await this.scheduler?.stopAutomation?.();
      if (this.server) await new Promise(resolve => this.server.close(() => resolve()));
      // Running jobs are marked interrupted on the next start and can be resumed from checkpoints.
      await this.db?.close?.();
    } catch (error) {
  ...
  async start() {
    ...
    if (!['127.0.0.1', 'localhost', '::1'].includes(HOST)) {
      this.logger.warn(`Listening on ${HOST}: the dashboard is reachable from other devices. Set ALLOWED_HOSTS to the hostname they use.`);
    }
    this.installProcessHandlers();
    this.server = this.app.listen(PORT, HOST, () => {
      console.log(chalk.green(`\n✅ YouTube Automation Agent running on ${HOST}:${PORT}`));
      console.log(chalk.gray(process.env.API_KEY
        ? '🔑 Mutating API routes require API_KEY from .env'
        : `🔑 Mutating API routes require the generated key in ${path.relative(__dirname, this.apiKeyPath)}`));
      console.log(chalk.gray('─'.repeat(50)));
