// Tests must never touch the real database, logs, API key file, or paid providers.
// These are set before any project module (or dotenv, which never overrides) is loaded.
const testSandbox = require('fs').mkdtempSync(require('path').join(require('os').tmpdir(), 'yaa-test-'));
process.env.DATABASE_PATH = require('path').join(testSandbox, 'test.db');
process.env.LOG_DIR = require('path').join(testSandbox, 'logs');
process.env.API_KEY_FILE = require('path').join(testSandbox, '.api-key');
process.env.CONFIG_DIR = require('path').join(testSandbox, 'config');
for (const key of ['OPENAI_API_KEY', 'OPENROUTER_API_KEY', 'MOONSHOT_API_KEY', 'MIMO_API_KEY', 'GLM_API_KEY',
  'GEMINI_API_KEY', 'ELEVENLABS_API_KEY', 'REPLICATE_API_TOKEN', 'REPLICATE_API_KEY', 'MINIMAX_API_KEY',
  'KLING_ACCESS_KEY', 'KLING_SECRET_KEY', 'DASHSCOPE_API_KEY', 'PEXELS_API_KEY', 'NOTIFICATION_WEBHOOK_URL', 'VIDEO_PROVIDER']) {
  process.env[key] = '';
}
...
class SystemTest {
  ...
  async runAllTests() {
    ...
    const tests = [
      ...
      { name: 'Measured Scene Timing', test: () => this.testMeasuredSceneTiming() },
      { name: 'AI Failure Handling', test: () => this.testAIFailureHandling() },
      { name: 'YouTube API Contract', test: () => this.testYouTubeAPIContract() },
      { name: 'Runtime Safety and Atomic State', test: () => this.testRuntimeSafety() },
      { name: 'Scheduler Settings', test: () => this.testSchedulerSettings() },
      ...
      { name: 'Local API Protection', test: () => this.testLocalAPIProtection() },
  ...
  async testOperatorWorkflowAPI() {
    ...
    try {
      ...
      agent.localApiKey = 'operator-workflow-test-key';
      agent.setupAPI();
      ...
      const authHeaders = { 'Content-Type': 'application/json', 'x-api-key': agent.getApiKey() };
  ...
  async testMeasuredSceneTiming() {
    const fs = require('fs').promises;
    const os = require('os');
    const {
      SceneRepairService, buildInitialSceneManifest, buildChapters, applyChaptersToDescription
    } = require('./utils/scene-repair-service');
    const { checkFFmpeg, runFFmpeg, getMediaDuration } = require('./utils/ffmpeg');

    const script = {
      title: 'Measured timing',
      hook: { text: 'Why do most budgets fail within a month?' },
      introduction: { greeting: 'Hello.', topicIntro: 'Today we cover budgets.', valueProposition: 'You will leave with a plan.', credibility: 'Using simple methods.' },
      mainContent: { sections: [
        { title: 'Track spending', content: ['Write down every purchase for two weeks.', 'Group purchases into categories.', 'Notice the categories that surprise you.'] },
        { title: 'Set limits', content: ['Pick three categories to cap.', 'Set a weekly number for each one.', 'Check the numbers every Sunday evening.'] },
        { title: 'Automate savings', content: ['Move money to savings on payday.', 'Treat it like a bill you must pay.', 'Increase the amount a little each quarter.'] }
      ] },
      conclusion: { recap: ['Track, limit, automate.'], finalThought: 'Small steps compound.' },
      callToAction: { type: 'call_to_action', subscribe: 'Subscribe for more.', like: 'Like if it helped.', comment: 'Tell me your budget tip.', nextVideo: 'Watch the next video.', duration: '15 seconds' }
    };

    // 1. Durations follow the measured narration and sum to it exactly, not to the script estimate.
    const manifest = buildInitialSceneManifest({ script, estimatedDuration: '7:35', assets: {} }, {}, { narrationDuration: 93.37 });
    const total = manifest.reduce((sum, scene) => sum + scene.duration, 0);
    if (Math.abs(total - 93.37) > 0.01) throw new Error(`Scene durations sum to ${total}s instead of the measured 93.37s`);
    const cta = manifest.find(scene => scene.label === 'Call to action');
    if (!cta || /call_to_action|15 seconds|next video/i.test(cta.scriptText)) {
      throw new Error('Call-to-action scene includes metadata that is never narrated');
    }
    if (manifest.some(scene => 'narratedText' in scene)) throw new Error('Internal weighting text leaked into the scene manifest');
    const estimated = buildInitialSceneManifest({ script, estimatedDuration: '7:35', assets: {} }, {});
    if (Math.abs(estimated.reduce((sum, scene) => sum + scene.duration, 0) - 455) > 0.01) {
      throw new Error('Unmeasured productions no longer fall back to the script estimate');
    }

    // 2. Chapters satisfy YouTube's rules: start at 00:00, at least three, each at least 10 seconds.
    const chapters = buildChapters(manifest);
    if (chapters.length < 3 || chapters[0].time !== '00:00' || chapters[0].title !== 'Introduction') {
      throw new Error(`Chapters do not start with a 00:00 introduction: ${JSON.stringify(chapters)}`);
    }
    const ends = [...chapters.slice(1).map(chapter => chapter.seconds), total];
    if (chapters.some((chapter, index) => ends[index] - chapter.seconds < 10)) throw new Error('A chapter is shorter than 10 seconds');
    if (chapters.some(chapter => /call to action/i.test(chapter.title))) throw new Error('Call to action was not folded into the previous chapter');
    if (buildChapters(manifest.slice(0, 2).map(scene => ({ ...scene, duration: 4 }))).length) {
      throw new Error('Chapters were produced for a timeline that cannot meet YouTube rules');
    }

    // 3. Estimated timestamps in descriptions are replaced with measured ones.
    const template = 'Intro line\n\n⏱️ TIMESTAMPS:\n00:00 Introduction\n00:20 Track spending\n01:50 Set limits\n\n📝 ABOUT THIS VIDEO:\nMore.';
    const updated = applyChaptersToDescription(template, chapters);
    if (updated.includes('01:50 Set limits') || !chapters.every(chapter => updated.includes(`${chapter.time} ${chapter.title}`)) || !updated.includes('📝 ABOUT THIS VIDEO:')) {
      throw new Error('Template timestamps were not replaced with measured chapters');
    }
    const aiDescription = 'Great video.\n0:00 Start\n2:10 Middle\n5:40 End\nThanks!';
    const aiUpdated = applyChaptersToDescription(aiDescription, chapters);
    if (aiUpdated.includes('5:40 End') || !aiUpdated.startsWith('Great video.') || !aiUpdated.endsWith('Thanks!')) {
      throw new Error('Invented timestamps in an AI description were not replaced');
    }
    if (applyChaptersToDescription(template, []).includes('TIMESTAMPS')) throw new Error('Unusable chapters left a stale timestamp block');

    // 4. With real audio, narration segments are cut at measured offsets and cover the whole file.
    if (!(await checkFFmpeg())) {
      this.logger.warn('FFmpeg unavailable — skipping real-audio scene timing assertions');
      return;
    }
    const directory = await fs.mkdtemp(path.join(os.tmpdir(), 'yaa-scene-timing-'));
    try {
      const audioPath = path.join(directory, 'narration.mp3');
      await runFFmpeg(['-y', '-f', 'lavfi', '-i', 'sine=frequency=330:duration=37', '-c:a', 'libmp3lame', audioPath]);
      const saved = [];
      const service = new SceneRepairService({
        replaceProductionScenes: async (_id, scenes) => { saved.push(...scenes); return scenes; }
      }, {
        isUsableAudioFile: async filePath => Boolean(filePath && await fs.stat(filePath).then(stat => stat.size > 0).catch(() => false))
      }, { dataRoot: directory, logger: this.logger });
      const production = { id: 'prod_timing', script, estimatedDuration: '7:35', assets: { audio: { path: audioPath, provider: 'fixture-tts' } } };
      await service.initializeProduction(production, {});
      const sceneTotal = saved.reduce((sum, scene) => sum + scene.duration, 0);
      if (Math.abs(sceneTotal - 37) > 0.2) throw new Error(`Scenes span ${sceneTotal}s but the narration is 37s`);
      let segmentTotal = 0;
      for (const scene of saved) {
        if (scene.narrationStatus !== 'current' || !scene.audioPath) throw new Error(`Scene ${scene.label} has no narration segment`);
        segmentTotal += await getMediaDuration(scene.audioPath) || 0;
      }
      if (Math.abs(segmentTotal - 37) > 1) throw new Error(`Narration segments cover ${segmentTotal.toFixed(1)}s of 37s`);
    } finally {
      await fs.rm(directory, { recursive: true, force: true }).catch(() => {});
    }
    this.logger.info('Measured scene timing test completed successfully');
  }

  async testSchedulerSettings() {
    const { PublishingSchedulingAgent } = require('./agents/publishing-scheduling-agent');
    const { YouTubeAutomationAgent } = require('./index');
    const settings = { channel_timezone: 'Asia/Karachi', auto_publish_enabled: 'false', max_daily_posts: '2' };
    let published = 0;
    const fakeDb = {
      getSetting: async key => settings[key] ?? null,
      countPublishedSince: async () => 1,
      updateScheduleEntry: async () => {}
    };
    const automation = new DailyAutomation({ publishing: { processPublishQueue: async () => { published++; return 0; } } }, fakeDb);
    try {
      // Cron runs in the channel timezone with overlap protection, and reports real task status.
      await automation.setupScheduledTasks();
      if (automation.timezone !== 'Asia/Karachi' || automation.scheduledTasks.size !== 8) throw new Error('Tasks did not use the channel timezone');
      const health = await automation.performHealthCheck();
      if (Object.values(health.scheduledTasks).some(running => !running)) throw new Error('Health check misreported running tasks');
      settings.channel_timezone = 'Not/AZone';
      if (await automation.resolveTimezone() !== undefined) throw new Error('An invalid timezone was accepted');

      // auto_publish_enabled=false really stops automatic uploads.
      await automation.processPublishQueue();
      if (published !== 0) throw new Error('auto_publish_enabled=false did not stop the publish queue');
      settings.auto_publish_enabled = 'true';
      await automation.processPublishQueue();
      if (published !== 1) throw new Error('Publish queue did not run once re-enabled');
    } finally {
      await automation.stopAutomation();
    }

    // max_daily_posts caps uploads per rolling 24 hours.
    const publisher = new PublishingSchedulingAgent(fakeDb, {});
    const uploaded = [];
    publisher.publishContent = async id => { uploaded.push(id); };
    const due = new Date(Date.now() - 60000).toISOString();
    publisher.publishQueue = ['a', 'b', 'c'].map(id => ({ productionId: id, title: id, status: 'scheduled', publishTime: due }));
    await publisher.processPublishQueue();
    if (uploaded.length !== 1) throw new Error(`max_daily_posts=2 with 1 already published allowed ${uploaded.length} uploads`);

    // Settings are validated before anything is written.
    const agent = new YouTubeAutomationAgent();
    const writes = [];
    agent.localApiKey = 'settings-test-key';
    agent.db = { setSetting: async (key, value) => writes.push([key, value]), getAllSettings: async () => ({}) };
    agent.setupAPI();
    const server = await new Promise(resolve => { const listener = agent.app.listen(0, '127.0.0.1', () => resolve(listener)); });
    try {
      const put = body => fetch(`http://127.0.0.1:${server.address().port}/api/settings`, {
        method: 'PUT', headers: { 'content-type': 'application/json', 'x-api-key': 'settings-test-key' }, body: JSON.stringify(body)
      });
      const rejected = await put({ auto_publish_enabled: 'false', channel_timezone: 'Mars/Olympus' });
      if (rejected.status !== 400 || writes.length) throw new Error('An invalid setting was partially written');
      const accepted = await put({ auto_publish_enabled: false, max_daily_posts: 3, posting_frequency: 'weekly' });
      if (!accepted.ok || writes.length !== 3 || !writes.some(([key, value]) => key === 'auto_publish_enabled' && value === 'false')) {
        throw new Error('Valid settings were not saved');
      }
    } finally {
      await new Promise(resolve => server.close(resolve));
    }
    this.logger.info('Scheduler settings test completed successfully');
  }

  async testRuntimeSafety() {
    const fs = require('fs').promises;
    const os = require('os');
    const { YouTubeAutomationAgent } = require('./index');
    const { AudienceEngagementService } = require('./utils/audience-engagement-service');
    const directory = await fs.mkdtemp(path.join(os.tmpdir(), 'yaa-runtime-'));
    const db = new Database();
    db.dbPath = path.join(directory, 'runtime.db');
    await db.initialize();
    const previousMax = process.env.MAX_CONCURRENT_JOBS;
    let server;
    try {
      // 1. A rejected async handler returns JSON 500 and the server keeps serving.
      const agent = new YouTubeAutomationAgent();
      agent.localApiKey = 'runtime-test-key';
      agent.db = { getGenerationJob: async () => { throw new Error('database is locked'); } };
      agent.logger = { error() {}, warn() {}, info() {} };
      agent.setupAPI();
      server = await new Promise(resolve => { const listener = agent.app.listen(0, '127.0.0.1', () => resolve(listener)); });
      const base = `http://127.0.0.1:${server.address().port}`;
      const failed = await fetch(`${base}/api/jobs/any`);
      const failedBody = await failed.json();
      if (failed.status !== 500 || failedBody.success !== false || /locked/.test(failedBody.error)) {
        throw new Error('An async route error was not converted into a safe JSON 500');
      }
      if (!(await fetch(`${base}/health`)).ok) throw new Error('Server stopped serving after a route error');

      // 2. Concurrent generation requests cannot exceed MAX_CONCURRENT_JOBS.
      process.env.MAX_CONCURRENT_JOBS = '1';
      const gen = new YouTubeAutomationAgent();
      gen.agents = { strategy: {} };
      gen.db = { createGenerationJob: async input => { await new Promise(resolve => setTimeout(resolve, 50)); return { id: `job_${Math.random()}`, ...input }; } };
      gen.runGenerationJob = () => new Promise(resolve => setTimeout(resolve, 100));
      const outcomes = await Promise.allSettled([gen.startGenerationJob({ topic: 'A' }), gen.startGenerationJob({ topic: 'B' })]);
      if (outcomes.filter(item => item.status === 'fulfilled').length !== 1 || outcomes.find(item => item.status === 'rejected')?.reason?.status !== 429) {
        throw new Error('Two concurrent requests both started with MAX_CONCURRENT_JOBS=1');
      }

      // 3. A stage update from a stale read cannot erase an operator's cancel request.
      const job = await db.createGenerationJob({ topic: 'Cancel race', source: 'manual' });
      const stale = await db.getGenerationJob(job.id);
      await db.updateGenerationJob(job.id, { cancelRequested: true, details: { cancelReason: 'Stopped by operator' } });
      const realGet = db.getGenerationJob.bind(db);
      let served = false;
      db.getGenerationJob = async id => { if (!served) { served = true; return stale; } return realGet(id); };
      await db.updateGenerationJob(job.id, { stage: 'script', progress: 25, details: { attempt: 1 } });
      db.getGenerationJob = realGet;
      const afterRace = await db.getGenerationJob(job.id);
      if (!afterRace.cancelRequested || afterRace.details.cancelReason !== 'Stopped by operator' || afterRace.details.attempt !== 1 || afterRace.stage !== 'script') {
        throw new Error(`Stage update erased the cancel request: ${JSON.stringify(afterRace)}`);
      }

      // 4. Double-approving a reply posts it once; an ambiguous failure blocks blind retries.
      let posts = 0;
      const engagement = new AudienceEngagementService(db, { hasYouTubeScope: () => true }, null, {
        logger: { warn() {}, info() {} },
        insertComment: async () => { posts++; await new Promise(resolve => setTimeout(resolve, 50)); return { id: `posted_${posts}` }; }
      });
      const draft = await db.saveReplyDraft({ commentId: 'comment_1', videoId: 'video_1', draftText: 'Thanks for watching!' });
      const approvals = await Promise.allSettled([
        engagement.approveReplyDraft(draft.id, { confirmed: true }),
        engagement.approveReplyDraft(draft.id, { confirmed: true })
      ]);
      if (posts !== 1 || approvals.filter(item => item.status === 'fulfilled').length !== 1 || (await db.getReplyDraft(draft.id)).status !== 'posted') {
        throw new Error('A double-approved reply was posted more than once');
      }
      const second = await db.saveReplyDraft({ commentId: 'comment_2', videoId: 'video_1', draftText: 'Great question!' });
      engagement.insertComment = async () => { posts++; throw Object.assign(new Error('socket hang up'), { code: 'ECONNRESET' }); };
      await engagement.approveReplyDraft(second.id, { confirmed: true }).catch(() => {});
      const retry = await engagement.approveReplyDraft(second.id, { confirmed: true }).then(() => null, error => error);
      if ((await db.getReplyDraft(second.id)).status !== 'uncertain' || retry?.code !== 'REPLY_ALREADY_CLAIMED' || posts !== 2) {
        throw new Error('An ambiguous posting failure allowed a blind retry');
      }
    } finally {
      if (server) await new Promise(resolve => server.close(resolve));
      if (previousMax === undefined) delete process.env.MAX_CONCURRENT_JOBS; else process.env.MAX_CONCURRENT_JOBS = previousMax;
      await db.close();
      await fs.rm(directory, { recursive: true, force: true }).catch(() => {});
    }
    this.logger.info('Runtime safety test completed successfully');
  }

  async testYouTubeAPIContract() {
    const fs = require('fs').promises;
    const os = require('os');
    const { AnalyticsOptimizationAgent } = require('./agents/analytics-optimization-agent');
    const { PublishingSchedulingAgent } = require('./agents/publishing-scheduling-agent');
    ...
    // Analytics: only metrics the Analytics API supports; unknown metrics make Google return 400.
    const supported = new Set(['views', 'estimatedMinutesWatched', 'averageViewDuration', 'averageViewPercentage',
      'viewerPercentage', 'subscribersGained', 'subscribersLost', 'estimatedRevenue', 'monetizedPlaybacks', 'playbackBasedCpm',
      'audienceWatchRatio', 'relativeRetentionPerformance', 'startedWatching', 'stoppedWatching', 'totalSegmentImpressions']);
    const requested = [];
    const agent = new AnalyticsOptimizationAgent({}, {});
    agent.youtubeAnalytics = { reports: { query: async params => {
      requested.push(params.metrics);
      const unknown = params.metrics.split(',').filter(metric => !supported.has(metric));
      if (unknown.length) throw Object.assign(new Error(`Unknown metric ${unknown.join(',')}`), { status: 400 });
      return { data: { rows: params.dimensions === 'day' ? [['2026-09-20', 40], ['2026-09-21', 60]] : [[120, 95, 41.5]] } };
    } } };
    agent.getVideoDetails = async () => ({ statistics: { viewCount: 100, likeCount: 5, commentCount: 1 } });
    const analytics = await agent.getVideoAnalytics('abc123', { startDate: '2026-09-20', endDate: '2026-09-21' });
    if (analytics.simulated || analytics.views.totalViews !== 100 || analytics.views.averageCTR !== null || analytics.views.totalImpressions !== null) {
      throw new Error(`Real analytics fell back or invented CTR (requested: ${requested.join(' | ')})`);
    }
    const thumbnail = await agent.analyzeThumbnailPerformance('abc123');
    const insights = await agent.generateInsights({}, analytics, thumbnail, { overallSEOScore: 60 });
    if (thumbnail.clickThroughRate !== null || insights.some(insight => insight.category === 'thumbnail')) {
      throw new Error('Unavailable CTR was treated as 0%');
    }
    const score = agent.calculatePerformanceScore(analytics);
    if (score.breakdown.ctr !== null) throw new Error('Unavailable CTR still counted in the performance score');

    // Learning keeps missing impressions/CTR as unavailable, not zero.
    const metrics = new ChannelLearningEngine({}).normalizeMetrics({ analytics, thumbnailMetrics: thumbnail });
    if (metrics.impressions !== null || metrics.ctr !== null) throw new Error('Learning stored missing CTR as a number');

    // Experiments refuse to measure without CTR evidence.
    const experiments = new GrowthExperimentService({}, { analyzeVideoPerformance: async () => ({ analytics, thumbnailMetrics: thumbnail }) }, {});
    const refused = await experiments.captureMetrics('abc123').then(() => null, error => error);
    if (refused?.code !== 'EXPERIMENT_EVIDENCE_UNAVAILABLE') throw new Error('Experiment measured CTR that does not exist');

    // Uploads never send publishAt (a past value publishes the video publicly at once).
    const directory = await fs.mkdtemp(path.join(os.tmpdir(), 'yaa-upload-'));
    try {
      const videoPath = path.join(directory, 'final.mp4');
      await fs.writeFile(videoPath, Buffer.from('fixture'));
      const publisher = new PublishingSchedulingAgent({ updateScheduleEntry: async () => {} }, {});
      let body = null;
      publisher.youtube = { videos: { insert: async params => { body = params.requestBody; return { data: { id: 'vid123' } }; } } };
      await publisher.uploadToYouTube({
        publishTime: new Date(Date.now() - 60000).toISOString(),
        metadata: { privacyStatus: 'private', video: { path: videoPath }, seo: { title: 'Private upload', description: 'Fixture', tags: ['a', 'b', 'c'] } }
      });
      if (!body || 'publishAt' in body.status || body.status.privacyStatus !== 'private') {
        throw new Error(`Upload request would change visibility: ${JSON.stringify(body?.status)}`);
      }
    } finally {
      await fs.rm(directory, { recursive: true, force: true });
    }
    this.logger.info('YouTube API contract test completed successfully');
  }

  async testAIFailureHandling() {
    ...
    const saved = ['OPENAI_API_KEY', 'OPENROUTER_API_KEY', 'MOONSHOT_API_KEY', 'MIMO_API_KEY', 'GLM_API_KEY', 'GEMINI_API_KEY']
      .map(key => [key, process.env[key]]);
    saved.forEach(([key]) => delete process.env[key]);
    try {
      const geminiService = responses => {
        const service = new AITextService({});
        let calls = 0;
        service.providerName = 'Fixture Gemini';
        service.model = 'fixture';
        service.gemini = { models: { generateContent: async () => {
      ...
      // 1. Transient 503s and empty bodies are retried; the eventual answer is returned.
      const flaky = geminiService([overloaded(), '', 'READY']);
      if (await flaky.service.generateText('x', { retryBaseMs: 1 }) !== 'READY' || flaky.calls() !== 3) {
        throw new Error('Transient AI failures were not retried to success');
      }
      // 2. A non-retryable request error fails immediately.
      const invalid = geminiService([new Error('{"error":{"code":400,"message":"Invalid argument"}}')]);
      await invalid.service.generateText('x', { retryBaseMs: 1 }).then(() => { throw new Error('400 succeeded'); }, () => {});
      if (invalid.calls() !== 1) throw new Error('A 400 error was retried');
      // 3. Hung requests time out.
      const hung = new AITextService({});
      hung.providerName = 'Fixture';
      hung.gemini = { models: { generateContent: () => new Promise(() => {}) } };
      const timedOut = await hung.generateText('x', { maxAttempts: 1, timeoutMs: 1000 }).catch(error => error);
      if (timedOut?.code !== 'ETIMEDOUT') throw new Error('A hung AI request did not time out');

      // 4. A configured provider that keeps failing fails the stage (retryable) — no template fallback.
      const db = { saveScript: async () => 'script', saveSEOData: async () => 'seo', getKeywordHistory: async () => [] };
      const strategy = { topic: 'Budgeting basics', angle: 'Simple steps', targetAudience: 'Beginners', contentType: 'Tutorial', keywords: ['budget'] };
      const writer = new ScriptWriterAgent(db, {});
      writer.aiTextService = geminiService([overloaded()]).service;
      const scriptFailure = await writer.generateScript(strategy).then(() => null, error => error);
      const recovery = new GenerationRecoveryService(db);
      if (!scriptFailure || scriptFailure.code !== 'AI_GENERATION_FAILED' || !recovery.isRetryable(scriptFailure)) {
        throw new Error('Script stage fell back to a template instead of failing retryably');
      }
      const seoAgent = new SEOOptimizerAgent(db, {});
      seoAgent.aiTextService = geminiService([overloaded()]).service;
      const seoFailure = await seoAgent.optimize({ title: 'Budgeting basics', mainContent: { sections: [] } }, strategy).then(() => null, error => error);
      if (seoFailure?.code !== 'AI_GENERATION_FAILED') throw new Error('SEO stage fell back to a template instead of failing');

      // 5. Without any provider, template output is marked, clean, and blocked from approval.
      const offlineWriter = new ScriptWriterAgent(db, {});
      const templateScript = await offlineWriter.generateScript(strategy);
      const offlineSEO = await new SEOOptimizerAgent(db, {}).optimize(templateScript, strategy);
      if (templateScript.metadata.generationSource !== 'template' || templateScript.introduction.credibility) {
        throw new Error('Template script was not marked or still contains a fabricated credibility line');
      }
      if (/\[(Specific|Another|Third)/.test(templateScript.fullScript) || /\[(Your|Related|Tool)/.test(offlineSEO.description) || /Audio Library/.test(offlineSEO.description)) {
        throw new Error('Template output still contains placeholder text or false credits');
      }
      const quality = await new OperatorService({ getRow: async () => ({ count: 0 }) }).runQualityChecks({
        script: templateScript, seo: offlineSEO, strategy, assets: {}
      }, {});
      if (!quality.blockingFailures.includes('script_source') || quality.blockingFailures.includes('placeholder_text')) {
        throw new Error('Quality gate did not block the template script');
      }
      const placeholderQuality = await new OperatorService({ getRow: async () => ({ count: 0 }) }).runQualityChecks({
        script: { fullScript: 'x'.repeat(300) }, seo: { title: 'T', description: `Links: [Your Channel URL] ${'y'.repeat(60)}` }, assets: {}
      }, {});
      if (!placeholderQuality.blockingFailures.includes('placeholder_text')) throw new Error('Placeholder text passed the quality gate');

      // 6. Slides carry no stray branding and escape markup from AI text.
      const html = new AIVideoGenerator({}).createSlideshowHTML({ title: '<img src=x onerror=alert(1)>', mainContent: { sections: [{ title: 'A', content: ['First real line.'] }] } }, []);
      if (/Ethereal|Content coming soon|<img src=x/.test(html) || !html.includes('First real line.')) {
        throw new Error('Slideshow HTML contains leftover branding, placeholders, or unescaped markup');
      }
    } finally {
      saved.forEach(([key, value]) => { if (value === undefined) delete process.env[key]; else process.env[key] = value; });
    }
    this.logger.info('AI failure handling test completed successfully');
  }
  ...
  async testDiscoverabilityPreflight() {
    ...
    try {
      ...
      apiAgent.localApiKey = 'discoverability-test-key';
      apiAgent.setupAPI();
      ...
      try {
        ...
        const apiHeaders = { 'content-type': 'application/json', 'x-api-key': apiAgent.getApiKey() };
  ...
  async testLocalAPIProtection() {
    const fs = require('fs').promises;
    const os = require('os');
    const { YouTubeAutomationAgent } = require('./index');
    const previousKey = process.env.API_KEY;
    const previousHosts = process.env.ALLOWED_HOSTS;
    delete process.env.API_KEY;
    delete process.env.ALLOWED_HOSTS;
    const directory = await fs.mkdtemp(path.join(os.tmpdir(), 'yaa-api-key-'));
    let server;
    try {
      const agent = new YouTubeAutomationAgent();
      agent.apiKeyPath = path.join(directory, '.api-key');
      agent.setupRequired = true;
      agent.db = { getSetting: async () => null };
      agent.setupAPI();

      // An empty API_KEY must generate and persist a strong key instead of disabling protection.
      const generated = agent.getApiKey();
      if (!/^[a-f0-9]{64}$/.test(generated) || (await fs.readFile(agent.apiKeyPath, 'utf8')).trim() !== generated) {
        throw new Error('A missing API_KEY did not produce a persisted random key');
      }
      const reloaded = new YouTubeAutomationAgent();
      reloaded.apiKeyPath = agent.apiKeyPath;
      if (reloaded.getApiKey() !== generated) throw new Error('The generated API key was not reused after restart');
      ...
      const noKey = await request('/generate', { method: 'POST', headers: { 'content-type': 'application/json' }, body: '{}' });
      if (noKey.status !== 401) throw new Error(`POST without a key returned ${noKey.status}, expected 401`);
      // A cross-site HTML form can only send simple content types and no custom headers.
      const formPost = await request('/api/automation/resume', { method: 'POST', headers: { 'content-type': 'text/plain' }, body: 'x' });
      if (formPost.status !== 401) throw new Error('A CSRF-style form post was not rejected');
      const wrongKey = await request('/api/settings', { method: 'PUT', headers: { 'content-type': 'application/json', 'x-api-key': 'wrong' }, body: '{}' });
      if (wrongKey.status !== 401) throw new Error('A wrong key was accepted');
      const withKey = await request('/generate', { method: 'POST', headers: { 'content-type': 'application/json', 'x-api-key': generated }, body: '{}' });
      if (withKey.status === 401) throw new Error('The generated key was rejected');

      const session = await request('/api/session');
      const sessionBody = await session.json();
      if (!session.ok || sessionBody.apiKey !== generated || session.headers.get('cache-control') !== 'no-store') {
        throw new Error('Loopback dashboard session did not receive the key');
      }
      const remote = agent.isLoopbackRequest({ socket: { remoteAddress: '192.168.1.40' } });
      if (remote || !agent.isLoopbackRequest({ socket: { remoteAddress: '::ffff:127.0.0.1' } })) {
        throw new Error('Loopback detection is wrong');
      }

      const health = await request('/health');
      if (!health.ok) throw new Error('Read-only routes should not require the key');

      // DNS rebinding: an attacker hostname resolving to 127.0.0.1 must be refused.
      const http = require('http');
      const rebound = await new Promise((resolve, reject) => {
        http.get({ host: '127.0.0.1', port: server.address().port, path: '/api/session', headers: { Host: 'attacker.example:1234' } },
          response => { response.resume(); resolve(response.statusCode); }).on('error', reject);
      });
      if (rebound !== 421) throw new Error(`Unexpected Host header returned ${rebound}, expected 421`);
    } finally {
      if (server) await new Promise(resolve => server.close(resolve));
      await fs.rm(directory, { recursive: true, force: true }).catch(() => {});
      if (previousKey === undefined) delete process.env.API_KEY; else process.env.API_KEY = previousKey;
      if (previousHosts === undefined) delete process.env.ALLOWED_HOSTS; else process.env.ALLOWED_HOSTS = previousHosts;
    }
    this.logger.info('Local API protection test completed successfully');
  }
  ...
  async testSlideshowRenderer() {
    ...
    try {
      ...
      // Crossfades must not shorten the slideshow below the requested length.
      const { getMediaDuration, runFFmpeg } = require('./utils/ffmpeg');
      const renderedDuration = await getMediaDuration(videoPath);
      if (!renderedDuration || Math.abs(renderedDuration - 6) > 0.5) {
        throw new Error(`Slideshow rendered ${renderedDuration}s instead of the requested 6s`);
      }

      // Regression: narration longer than the visuals must never be truncated by -shortest.
      const longNarration = path.join(dir, 'narration.mp3');
      await runFFmpeg(['-y', '-f', 'lavfi', '-i', 'sine=frequency=440:duration=10', '-c:a', 'libmp3lame', longNarration]);
      const narratedPath = path.join(dir, 'narrated.mp4');
      await generator.addAudioToVideo(videoPath, longNarration, narratedPath);
      const narratedDuration = await getMediaDuration(narratedPath);
      if (!narratedDuration || narratedDuration < 9.7) {
        throw new Error(`Final video is ${narratedDuration}s but narration is 10s — audio was truncated`);
      }
  ...
  async testReplyApprovalAndPosting() {
    ...
    try {
      ...
      const failing = new AudienceEngagementService(db, scopedCredentials, null, {
        // googleapis attaches the HTTP status; a definite 4xx rejection is safe to retry manually.
        insertComment: async () => { throw Object.assign(new Error('commentThreadNotFound'), { status: 404 }); }
      });
      status = null;
...
if (require.main === module) {
  ...
  tester.runAllTests()
    .then(success => {
      require('fs').rmSync(testSandbox, { recursive: true, force: true });
