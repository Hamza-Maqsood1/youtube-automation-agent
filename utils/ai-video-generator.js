...
// Slide text comes from AI output and trend titles; escape it before it becomes page markup.
...
class AIVideoGenerator {
    ...
    try {
      if (process.env.PEXELS_API_KEY) {
        ...
        for (let i = 0; i < count; i++) {
          ...
          await this.generatePexelsImage(enhancedPrompt, imagePath);
          localPaths.push(imagePath);
        }
        this.logger.info(`Pexels: fetched ${localPaths.length} visual assets`);
        return localPaths;
      }
  ...
  async generateImage(prompt, imagePath) {
    ...
    if (process.env.PEXELS_API_KEY) {
      return await this.generatePexelsImage(prompt, imagePath);
    }
  ...
  async generatePexelsImage(prompt, imagePath) {
    const https = require('https');
    const fsSync = require('fs');
    const query = encodeURIComponent(prompt.split(',')[0].trim().substring(0, 50));
      ...
      const req = https.get(url, {
        ...
        res.on('end', async () => {
          try {
            ...
            const photoUrl = json.photos?.[0]?.src?.large2x || json.photos?.[0]?.src?.original;
            if (!photoUrl) throw new Error('No Pexels photo found');
            ...
            this.logger.info(`Pexels image downloaded for: ${prompt.substring(0, 40)}`);
            resolve(imagePath);
          } catch (e) { reject(e); }
        });
      });
  ...
  async generateSlideshowVideo(script, visualAssets, audioPath, outputPath) {
    ...
    try {
      ...
      // The narration is the source of truth for length; the script word count is only a fallback.
      const narrationDuration = await this.isUsableAudioFile(audioPath) ? await getMediaDuration(audioPath) : null;
      const duration = narrationDuration || this.calculateScriptDuration(script);
  ...
  async renderSlidesToVideo(stills, totalDuration, videoPath) {
    ...
    const fade = 0.5;
    // Each crossfade overlaps two slides, so add the overlap back to hit the requested total.
    const overlap = stills.length > 1 ? (stills.length - 1) * fade : 0;
...
<body>
    ...
    <div class="slide active">
        ...
        <div class="content">
            ...
            ${process.env.CHANNEL_NAME ? `<p>${escapeHTML(process.env.CHANNEL_NAME)}</p>` : ''}
        </div>
    </div>
  ...
  formatSectionContent(section) {
    if (section.items && Array.isArray(section.items)) {
      return section.items.slice(0, 3).map(item =>
        `<p>${escapeHTML(item.number)}. ${escapeHTML(item.title)}</p>`
      ).join('');
    }
    ...
    if (typeof section.content === 'string') {
      return `<p>${escapeHTML(section.content.slice(0, 200))}${section.content.length > 200 ? '...' : ''}</p>`;
    }

    if (Array.isArray(section.content)) {
      return section.content
        .filter(line => typeof line === 'string' && line.trim() && !line.startsWith('['))
        .slice(0, 3)
        .map(line => `<p>${escapeHTML(line.length > 120 ? `${line.slice(0, 117)}...` : line)}</p>`)
        .join('');
    }
  ...
  calculateScriptDuration(script) {
    ...
    // Count everything formatScriptForTTS narrates, so the fallback tracks the real voiceover.
    const count = value => String(value || '').split(/\s+/).filter(Boolean).length;
    let totalWords = 0;

    if (script.hook) totalWords += count(script.hook.text);
    if (script.introduction) {
      const intro = script.introduction;
      totalWords += count(intro.greeting) + count(intro.topicIntro) + count(intro.valueProposition) + count(intro.credibility);
    }

    for (const section of script.mainContent?.sections || []) {
      totalWords += count(section.title);
      if (typeof section.content === 'string') {
        totalWords += count(section.content);
      } else if (Array.isArray(section.content)) {
        section.content.forEach(line => { totalWords += count(line); });
      }
      (section.items || []).forEach(item => { totalWords += count(`${item.title} ${item.description}`); });
      (section.steps || []).forEach(step => { totalWords += count(`${step.title} ${step.description} ${step.tip || ''}`); });
    }

    if (script.conclusion) {
      (script.conclusion.recap || []).forEach(line => { totalWords += count(line); });
      totalWords += count(script.conclusion.finalThought);
    }
    if (script.callToAction) {
      const cta = script.callToAction;
      totalWords += count(cta.subscribe) + count(cta.like) + count(cta.comment);
    }
  ...
  async addAudioToVideo(videoPath, audioPath, outputPath, options = {}) {
    ...
    // `-shortest` trims to the shorter stream, so a visual track that is shorter than the
    // narration would cut the voiceover mid-sentence. Hold the last frame instead.
    const videoInput = options.loopVideo ? ['-stream_loop', '-1', '-i', videoPath] : ['-i', videoPath];
    let videoCodec = ['-c:v', 'copy'];
    if (!options.loopVideo) {
      const [videoDuration, audioDuration] = await Promise.all([getMediaDuration(videoPath), getMediaDuration(audioPath)]);
      if (videoDuration && audioDuration && audioDuration > videoDuration + 0.05) {
        const padSeconds = (audioDuration - videoDuration + 0.5).toFixed(2);
        this.logger.warn(`Visual track is ${(audioDuration - videoDuration).toFixed(1)}s shorter than the narration; holding the last frame to avoid truncating audio.`);
        videoCodec = ['-vf', `tpad=stop_mode=clone:stop_duration=${padSeconds}`, '-c:v', 'libx264', '-pix_fmt', 'yuv420p'];
      }
    }
    await runFFmpeg(['-y', ...videoInput, '-i', audioPath, '-map', '0:v:0', '-map', '1:a:0', ...videoCodec, '-c:a', 'aac', '-shortest', muxPath]);
