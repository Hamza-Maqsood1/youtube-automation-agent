...
function scriptScenes(script = {}) {
  ...
  for (const [index, section] of (script.mainContent?.sections || []).entries()) {
    ...
    scenes.push({
      ...
      scriptText,
      // The full-production narration also speaks the section heading (see formatScriptForTTS).
      narratedText: `Section ${index + 1}: ${section.title || ''} ${scriptText}`,
  ...
  if (script.callToAction) {
    // Only the lines the narration actually speaks — not type/duration metadata or nextVideo.
    const cta = script.callToAction;
    const scriptText = [cta.subscribe, cta.like, cta.comment].filter(value => typeof value === 'string' && value.trim()).join(' ');
    if (scriptText) scenes.push({
...
function sceneDurations(blueprints, totalDuration) {
  // Weight by the words the narration speaks for each scene, then make the durations sum
  // exactly to the total so the scene timeline lines up with the audio end to end.
  const weights = blueprints.map(scene => Math.max(1, String(scene.narratedText || scene.scriptText || '').trim().split(/\s+/).filter(Boolean).length));
  const totalWeight = weights.reduce((sum, weight) => sum + weight, 0);
  const durations = weights.map(weight => Number(((weight / totalWeight) * totalDuration).toFixed(2)));
  const assigned = durations.slice(0, -1).reduce((sum, value) => sum + value, 0);
  durations[durations.length - 1] = Number(Math.max(0.5, totalDuration - assigned).toFixed(2));
  return durations;
}

function buildInitialSceneManifest(production = {}, providerResult = {}, options = {}) {
  const blueprints = scriptScenes(production.script || {});
  // Measured narration length is authoritative; the script estimate is only a fallback.
  const measured = Number(options.narrationDuration);
  const totalDuration = Number.isFinite(measured) && measured > 0
    ? measured
    : durationSeconds(production.estimatedDuration || production.assets?.finalVideo?.duration, Math.max(30, blueprints.length * 8));
  const durations = sceneDurations(blueprints, totalDuration);
  ...
  return blueprints.map(({ narratedText: _narratedText, ...scene }, position) => {
...
/**
 * Derive YouTube chapters from the measured scene timeline. YouTube only shows chapters when
 * the first starts at 00:00, there are at least three, and each lasts at least 10 seconds,
 * so hook/intro are merged, the call to action folds into the scene before it, and short
 * chapters merge into their neighbour. Returns [] when the rules cannot be met.
 */
function buildChapters(scenes = []) {
  const chapters = [];
  let cursor = 0;
  for (const scene of [...scenes].sort((a, b) => Number(a.position) - Number(b.position))) {
    const duration = Math.max(0, Number(scene.duration || 0));
    const label = String(scene.label || '').trim();
    const opening = /^(hook|introduction)$/i.test(label);
    const previous = chapters.at(-1);
    if (previous && ((opening && previous.opening) || /^call to action$/i.test(label))) {
      previous.duration += duration;
    } else {
      chapters.push({ title: opening ? 'Introduction' : label || 'Section', seconds: cursor, duration, opening });
    }
    cursor += duration;
  }

  for (let index = 0; index < chapters.length && chapters.length > 1;) {
    if (chapters[index].duration >= MIN_CHAPTER_SECONDS) {
      index++;
      continue;
    }
    if (index === 0) {
      chapters[1].seconds = 0;
      chapters[1].duration += chapters[0].duration;
  ...
  if (chapters.length < 3 || chapters.some(chapter => chapter.duration < MIN_CHAPTER_SECONDS)) return [];
  return chapters.map(chapter => ({
    time: formatChapterTime(chapter.seconds),
    title: chapter.title,
    seconds: Number(chapter.seconds.toFixed(2))
  }));
}

/**
 * Replace estimated timestamps in a description with measured chapters: the template's
 * TIMESTAMPS block if present, otherwise the first run of timestamp lines, otherwise append.
 */
function applyChaptersToDescription(description = '', chapters = []) {
  const lines = String(description || '').split('\n');
  const chapterLines = chapters.map(chapter => `${chapter.time} ${chapter.title}`);
  const markerIndex = lines.findIndex(line => line.trim() === '⏱️ TIMESTAMPS:');
  let start = markerIndex >= 0 ? markerIndex + 1 : lines.findIndex(line => TIMESTAMP_LINE.test(line));
  let end = start;
  if (start >= 0) {
    while (end < lines.length && TIMESTAMP_LINE.test(lines[end])) end++;
  }

  let next;
  if (markerIndex >= 0) {
    next = chapterLines.length
      ? [...lines.slice(0, start), ...chapterLines, ...lines.slice(end)]
      : [...lines.slice(0, markerIndex), ...lines.slice(end).slice(lines[end] === '' ? 1 : 0)];
  } else if (start >= 0 && end - start >= 2) {
    next = [...lines.slice(0, start), ...chapterLines, ...lines.slice(end)];
  } else if (chapterLines.length) {
    next = [...lines, '', 'Chapters:', ...chapterLines];
  } else {
    return String(description || '');
  }
  const output = next.join('\n').trim();
  return output.length <= 5000 ? output : String(description || '');
}
...
class SceneRepairService {
  ...
  async ensureManifest(bundle) {
    ...
    const narrationDuration = await this.measureAudio(bundle.assets?.audio?.path);
    const scenes = buildInitialSceneManifest(bundle, bundle.assets?.finalVideo?.provider || {}, { narrationDuration });
  ...
  async initializeProduction(production, providerResult = {}) {
    const narrationDuration = await this.measureAudio(production.assets?.audio?.path);
    const scenes = buildInitialSceneManifest(production, providerResult, { narrationDuration });
  ...
  // Real audio length in seconds, or null when the file is missing or cannot be decoded.
  async measureAudio(audioPath) {
    if (!await this.videoGenerator?.isUsableAudioFile?.(audioPath)) return null;
    return getMediaDuration(audioPath).catch(() => null);
  }
  ...
  async regenerateNarration(productionId, sceneId, input = {}) {
    ...
    try {
      ...
      const measured = await this.measureAudio(generatedPath);
      const next = await this.db.updateProductionScene(productionId, sceneId, {
        // New narration rarely matches the old length; the rebuild trims/pads audio to scene.duration.
        ...(measured ? { duration: Math.ceil(measured * 100) / 100 } : {}),
  ...
  async rebuild(productionId) {
    ...
    await this.syncChapters(bundle, scenes);
  ...
  // Keep description timestamps and seo.chapters aligned with the rebuilt timeline.
  async syncChapters(bundle, scenes) {
    const chapters = buildChapters(scenes);
    const seo = {
      ...(bundle.seo || {}),
      chapters,
      description: applyChaptersToDescription(bundle.seo?.description, chapters)
    };
    await this.db.saveProductionSnapshot({
      id: bundle.id, strategy: bundle.strategy, script: bundle.script, thumbnail: bundle.thumbnail, seo
    });
    if (bundle.editorData?.description) {
      await this.db.saveContentReview(bundle.id, {
        status: bundle.review_status || 'needs_review',
        editorData: { ...bundle.editorData, description: applyChaptersToDescription(bundle.editorData.description, chapters) },
...
module.exports = {
  SceneRepairService,
  buildInitialSceneManifest,
  scriptScenes,
  buildChapters,
  applyChaptersToDescription
};
