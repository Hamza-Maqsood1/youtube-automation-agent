...
class GrowthExperimentService {
  ...
  async captureMetrics(videoId) {
    ...
    const rawCtr = report.thumbnailMetrics?.clickThroughRate ?? views.averageCTR;
    const rawImpressions = report.thumbnailMetrics?.impressions ?? views.totalImpressions;
    if (rawCtr === null || rawCtr === undefined || rawImpressions === null || rawImpressions === undefined) {
      throw this.error(
        'Thumbnail impressions and CTR are unavailable from the YouTube Analytics API; a packaging experiment cannot be measured. Use YouTube Studio "Test & Compare" instead.',
        409,
