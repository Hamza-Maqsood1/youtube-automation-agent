...
class ChannelLearningEngine {
  ...
  normalizeMetrics(report, context = {}) {
    ...
    // Impressions/CTR are not exposed by the YouTube Analytics API; keep them null (unavailable), never 0.
  ...
  confidenceFor(metrics) {
    if (metrics.impressions === null || metrics.impressions === undefined) {
      if (metrics.views >= 500) return 'high';
      if (metrics.views >= 50) return 'medium';
      return 'low';
    }
  ...
  buildChannelRecommendations(snapshots) {
    const recommendations = [];
    // CTR needs real impression data; retention only needs enough real views.
    const sufficientlyExposed = snapshots.filter(snapshot => this.optionalNumber(snapshot.metrics?.impressions) >= 100 && this.optionalNumber(snapshot.metrics?.ctr) !== null);
    const sufficientlyViewed = snapshots.filter(snapshot => this.number(snapshot.metrics?.views) >= 50);
    const averageCTR = this.average(sufficientlyExposed.map(item => this.number(item.metrics.ctr)));
    const averageRetention = this.average(sufficientlyViewed.map(item => this.number(item.metrics.retention)));

    if (sufficientlyExposed.length >= 2 && averageCTR < 4) {
      recommendations.push({
        category: 'packaging',
    ...
    if (sufficientlyViewed.length >= 2 && averageRetention < 35) {
      recommendations.push({
        category: 'retention',
        title: 'Tighten hooks and early pacing',
        rationale: `Average view percentage was ${averageRetention.toFixed(1)}% across ${sufficientlyViewed.length} videos with at least 50 views.`,
        evidence: { metric: 'retention', average: averageRetention, sampleSize: sufficientlyViewed.length, minimumViews: 50 },
        proposedChange: { target: 'future_scripts', prefer: 'concise_hook_and_faster_opening' },
        confidence: sufficientlyViewed.length >= 5 ? 'high' : 'medium'
      });
    }
    return recommendations;
  }
