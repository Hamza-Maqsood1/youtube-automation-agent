...
// Unfilled template slots such as "[Your Channel URL]" or "[Related Video 1]".
const PLACEHOLDER_PATTERN = /\[(?:your|related video|tool\/resource|specific|another relevant|third compelling|insert|placeholder|todo|tbd)\b[^\]]{0,60}\]/i;
...
class OperatorService {
  ...
  async runQualityChecks(production, profile = {}) {
    ...
    // Template scripts are filler (generic lines, no real content); they may be produced when no
    // AI provider is configured, but they must never reach approval or publishing.
    const templateScript = production.script?.metadata?.generationSource === 'template';
    checks.push(this.check('script_source', !templateScript,
      templateScript
        ? 'Script came from the offline template, not an AI provider; configure an AI text provider and regenerate'
        : 'Script was written by the configured AI provider'));
    const placeholders = `${title}\n${description}`.match(PLACEHOLDER_PATTERN);
    checks.push(this.check('placeholder_text', !placeholders,
      placeholders
        ? `Title or description still contains placeholder text: ${placeholders[0]}`
