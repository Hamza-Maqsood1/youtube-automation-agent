...
Comments: ${JSON.stringify(payload)}`;
  ...
  async updateReplyDraft(draftId, changes = {}) {
    ...
    if (['posted', 'posting'].includes(draft.status)) {
      const error = new Error(draft.status === 'posting' ? 'This reply is being posted right now' : 'A posted reply cannot be changed');
      error.status = 409;
      throw error;
    }
  ...
  async approveReplyDraft(draftId, input = {}) {
    ...
    if (!await this.db.claimReplyDraftForPosting(draftId)) {
      const current = await this.db.getReplyDraft(draftId);
      const error = new Error(current?.status === 'uncertain'
        ? 'A previous attempt may already have posted this reply. Check the video on YouTube, then edit the draft to retry.'
        : `This reply is already ${current?.status || 'being posted'}`);
      error.status = 409;
      error.code = 'REPLY_ALREADY_CLAIMED';
      throw error;
    }
    ...
    try {
      ...
      if (!posted?.id) throw Object.assign(new Error('YouTube did not return a comment id for the posted reply'), { outcomeUnknown: true });
      ...
      // A timeout, network drop, or 5xx may still have created the comment. Mark it 'uncertain'
      // so it cannot be re-approved blindly; a definite rejection (4xx) can be retried.
      const status = Number(error.status || error.code || error.response?.status || 0);
      const uncertain = Boolean(error.outcomeUnknown || !status || status >= 500);
      await this.db.updateReplyDraft(draftId, {
        status: uncertain ? 'uncertain' : 'failed',
        failureReason: String(uncertain
          ? `Outcome unknown (${error.message || 'no response'}); check YouTube before retrying`
          : error.message || 'Posting failed').slice(0, 300)
      });
