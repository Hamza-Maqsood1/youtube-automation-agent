...
// The server hands its key to a dashboard opened on the same machine; remote browsers get a
// 403 here and fall back to entering the key manually.
async function loadSessionKey() {
  try {
    const response = await fetch('/api/session', { cache: 'no-store' });
    if (!response.ok) return false;
    const data = await response.json();
    if (!data.apiKey || data.apiKey === apiKey()) return false;
    localStorage.setItem('yaa_api_key', data.apiKey);
    return true;
  } catch (_error) {
    return false;
  }
...
async function api(url, options = {}, retry = true) {
  ...
  if (response.status === 401 && retry && (await loadSessionKey() || requestApiKey() !== null)) return api(url, options, false);
...
function renderLearning(learning = {}) {
  ...
  $('#learning-recommendations').innerHTML = recommendations.length ? recommendations.map(item => `
    ...
    </article>`).join('') : empty('No recommendation yet. The agent needs at least two real, sufficiently exposed measurements.');
}
...
function switchView(view) {
  ...
  const titles = {
    ...
    operator: ['AUTONOMOUS OPERATOR', 'Configure your channel strategy.'],
...
document.addEventListener('click', async event => {
  ...
  if (experiment) {
    ...
    const prompts = {
      ...
      start: 'Start this live test? YT Agent will rotate only the approved arms and restore the control before asking you to adopt a winner.',
