...
class DailyAutomation {
  ...
  // Scheduled jobs run in the channel timezone and never overlap a still-running previous run
  // (e.g. a slow publish-queue pass is not started again 15 minutes later).
  async resolveTimezone() {
    const configured = (this.db.getSetting ? await this.db.getSetting('channel_timezone') : null) || process.env.CHANNEL_TIMEZONE;
    if (!configured) return undefined;
    try {
      new Intl.DateTimeFormat('en-US', { timeZone: configured });
      return configured;
    } catch (_error) {
      this.logger.warn(`Ignoring invalid channel timezone "${configured}"; using the server timezone`);
      return undefined;
    }
  }

  async isSettingEnabled(key) {
    return (this.db.getSetting ? await this.db.getSetting(key) : null) !== 'false';
  }

  // Re-create the cron tasks, e.g. after the channel timezone changes.
  ...
  async setupScheduledTasks() {
    ...
    const tasks = [
      // Content generation at 6:00 AM
      ['daily-content-generation', '0 6 * * *', () => this.runDailyContentGeneration()],
      // Publishing queue every 15 minutes
      ['publish-queue-processing', '*/15 * * * *', () => this.processPublishQueue()],
      // Analytics collection at 9:00 AM
      ['daily-analytics', '0 9 * * *', () => this.collectDailyAnalytics()],
      // Weekly strategy review on Sundays at 8:00 AM
      ['weekly-strategy-review', '0 8 * * 0', () => this.weeklyStrategyReview()],
      // Optimization tasks at 10:00 PM
      ['daily-optimization', '0 22 * * *', () => this.runDailyOptimization()],
      // Database maintenance on Saturdays at 3:00 AM
      ['database-maintenance', '0 3 * * 6', () => this.databaseMaintenance()],
      // Audience comment sync every 4 hours; the service's own taper decides which videos are due
      ['audience-engagement-sync', '0 */4 * * *', () => this.collectAudienceEngagement()],
      // Collect controlled experiment evidence and advance only pre-approved arms
      ['growth-experiment-refresh', '30 */4 * * *', () => this.refreshGrowthExperiments()]
    ];
    for (const [name, expression, run] of tasks) {
      const task = cron.createTask(expression, async () => {
        if (this.isEnabled) await run();
      }, { name, timezone: this.timezone, noOverlap: true });
      this.scheduledTasks.set(name, task);
      task.start();
      this.logger.info(`Started scheduled task: ${name}${this.timezone ? ` (${this.timezone})` : ''}`);
    }
  }

  async runDailyContentGeneration() {
    try {
      if (!await this.isSettingEnabled('daily_content_enabled')) {
        this.logger.info('Daily content generation is disabled (daily_content_enabled=false)');
        return;
      }
  ...
  async processPublishQueue() {
    try {
      if (!await this.isSettingEnabled('auto_publish_enabled')) return;
  ...
  async collectDailyAnalytics() {
    try {
      if (!await this.isSettingEnabled('analytics_enabled')) {
        this.logger.info('Analytics collection is disabled (analytics_enabled=false)');
        return;
      }
  ...
  async weeklyStrategyReview() {
    try {
      ...
      // Only moves already-approved publish times when the operator has opted in.
      if ((this.db.getSetting ? await this.db.getSetting('publish_time_optimization') : null) === 'true') {
  ...
  async runDailyOptimization() {
    try {
      if (!await this.isSettingEnabled('optimization_enabled')) {
        this.logger.info('Daily optimization is disabled (optimization_enabled=false)');
        return;
      }
  ...
  async databaseMaintenance() {
    try {
      ...
      // Create backup
      const backupPath = await this.isSettingEnabled('content_backup_enabled') ? await this.db.backup() : null;
      if (backupPath) this.logger.info(`Database backed up to: ${backupPath}`);
  ...
  async performHealthCheck() {
    ...
    this.scheduledTasks.forEach((task, name) => {
      health.scheduledTasks[name] = task.getStatus ? task.getStatus() !== 'stopped' : Boolean(task.running);
    });
  ...
  async getAutomationStatus() {
    return {
      ...
      scheduledTasks: Array.from(this.scheduledTasks.keys()).map(name => ({
        name,
        running: this.scheduledTasks.get(name).getStatus?.() !== 'stopped'
      })),
