...
const { SceneRepairService, buildChapters, applyChaptersToDescription } = require('../utils/scene-repair-service');
...
class ProductionManagementAgent {
  ...
  async processContent(contentData) {
    try {
      ...
      // Scene durations are split from the measured narration, so captions and chapters
      // are derived from the manifest afterwards rather than from script estimates.
      const scenes = await this.sceneRepair.initializeProduction(productionData, this.aiVideoGenerator.lastVideoResult || {});
      const narrationSeconds = await this.sceneRepair.measureAudio(productionData.assets.audio?.path);
      await this.generateCaptions(productionData, narrationSeconds ? scenes : null);
      this.applyMeasuredTiming(productionData, narrationSeconds ? scenes : null, narrationSeconds);
  ...
  async generateCaptions(productionData, measuredScenes = null) {
    this.logger.info('Generating captions...');

    const captionsPath = path.join(__dirname, '..', 'data', 'captions', `${productionData.id}_captions.srt`);

    // Prefer the measured scene timeline; the script-estimate timing is only a fallback
    // for productions without usable narration.
    const sceneAware = Array.isArray(measuredScenes) && measuredScenes.length > 0;
    const captions = sceneAware
      ? this.sceneRepair.buildSRT(measuredScenes)
      : await this.createSRTCaptions(productionData);
  ...
  // Replace the SEO stage's estimated chapters/timestamps and the estimated durations with
  // values measured from the real narration. No-op when narration could not be measured.
  applyMeasuredTiming(productionData, measuredScenes, narrationSeconds) {
    if (!narrationSeconds || !Array.isArray(measuredScenes) || !measuredScenes.length) return;
    const chapters = buildChapters(measuredScenes);
    productionData.seo = {
      ...(productionData.seo || {}),
      chapters,
      description: applyChaptersToDescription(productionData.seo?.description, chapters)
    };
    const wholeSeconds = Math.round(narrationSeconds);
    const measured = `${Math.floor(wholeSeconds / 60)}:${String(wholeSeconds % 60).padStart(2, '0')}`;
    if (productionData.assets.audio) productionData.assets.audio.duration = measured;
    if (productionData.assets.finalVideo && !productionData.assets.finalVideo.simulated) {
      productionData.assets.finalVideo.duration = measured;
    }
  }
