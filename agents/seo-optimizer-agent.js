...
Keep tags under YouTube's 500 character total guidance. Avoid fabricated statistics and unsupported claims.`;
    ...
    try {
      ...
      throw providerFailure('SEO optimization', this.aiTextService.providerName, error);
    }
  }
  ...
  async generateDescription(script, strategy) {
    ...
    // Only real, configured links — never "[Your Website]"-style placeholders.
    const links = [
      ['Website', process.env.WEBSITE_URL],
      ['Social Media', process.env.SOCIAL_LINKS]
    ].filter(([, value]) => value);
    if (links.length) {
      description += '🔗 USEFUL LINKS:\n';
      links.forEach(([name, value]) => { description += `• ${name}: ${value}\n`; });
      description += '\n';
    }
