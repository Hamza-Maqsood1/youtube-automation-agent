...
const { AITextService, providerFailure } = require('../utils/ai-text-service');
...
Avoid fabricated statistics, unsupported claims, and fake urgency. List every externally verifiable factual claim in claims. Use only exact URLs from Research sources; use an empty sourceUrls array when the supplied sources do not support a claim.`;
    ...
    try {
      ...
      // A configured provider that keeps failing must fail the job (resumable) rather than
      // silently shipping template filler.
      throw providerFailure('Script generation', this.aiTextService.providerName, error);
    }
  }
  ...
  async generateExamples(strategy) {
    return {
      ...
      content: [
        `Let's look at how ${strategy.topic} plays out in practice.`,
