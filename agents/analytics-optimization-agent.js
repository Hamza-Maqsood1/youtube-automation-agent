...
class AnalyticsOptimizationAgent {
  ...
  async getViewsAnalytics(videoId, startDate, endDate) {
    const response = await this.youtubeAnalytics.reports.query({
      ...
      startDate,
      endDate,
      // The Analytics API has no thumbnail impression/CTR metrics (those live in the Reporting
      // API's channel_reach_basic_a1 bulk report). Requesting them made every query fail and
      // silently turned all analytics into simulated data.
      metrics: 'views',
      dimensions: 'day',
  ...
  async analyzeThumbnailPerformance(_videoId, _period = null) {
    // Thumbnail impressions and CTR are not available from the YouTube Analytics API.
    // Report them as unavailable (null) rather than 0 so nothing learns from missing data.
    return {
      available: false,
      impressions: null,
      clickThroughRate: null,
      ctrQuality: 'unavailable',
      recommendations: ['Thumbnail impressions and CTR are not exposed by the YouTube Analytics API; check YouTube Studio.']
    };
  }
  ...
  async generateInsights(videoDetails, analytics, thumbnailMetrics, seoMetrics) {
    ...
    // Thumbnail insights (only when CTR evidence exists; null must not read as 0% CTR)
    if (thumbnailMetrics.clickThroughRate === null || thumbnailMetrics.clickThroughRate === undefined) {
      // No CTR evidence available.
  ...
  calculatePerformanceScore(analytics) {
    ...
    // CTR score (20 points max) — excluded from the scale when CTR is unavailable
    const ctrAvailable = Number.isFinite(analytics.views?.averageCTR);
    const ctrScore = ctrAvailable ? Math.min(20, analytics.views.averageCTR * 2) : 0;
    score += ctrScore;
    if (ctrAvailable) maxScore += 20;
    ...
    return {
      ...
      breakdown: {
        ...
        ctr: ctrAvailable ? Math.round(ctrScore) : null
      },
