...
class PublishingSchedulingAgent {
  ...
  async uploadToYouTube(scheduleEntry) {
    ...
    const videoMetadata = {
      ...
      status: {
        // The queue uploads when publishTime arrives, so the chosen privacy applies immediately.
        // Never send status.publishAt here: a past publishAt makes YouTube publish the video
        // publicly right away, even when privacyStatus is "private".
  ...
  async processPublishQueue() {
    ...
    // max_daily_posts caps automatic uploads per rolling 24 hours (0 or unset = no limit);
    // anything over the cap stays scheduled and goes out on a later pass.
    const cap = parseInt(this.db.getSetting ? await this.db.getSetting('max_daily_posts') : '', 10);
    let batch = readyToPublish;
    if (cap > 0 && this.db.countPublishedSince) {
      const publishedToday = await this.db.countPublishedSince(new Date(Date.now() - 86400000).toISOString());
      batch = readyToPublish.slice(0, Math.max(0, cap - publishedToday));
      if (batch.length < readyToPublish.length) {
        this.logger.info(`Daily upload cap reached (${publishedToday}/${cap}); ${readyToPublish.length - batch.length} item(s) wait for a later pass`);
      }
      if (!batch.length) return 0;
    }

    this.logger.info(`Processing publish queue: ${batch.length} item(s) ready to publish...`);
